...
\caption{Reranking-strength sensitivity, NDCG@20, five-seed means (Table~\ref{tab:ablation_lambda}). ML-1M: matched v8 execution, all eight families on identical models; LOO is the best family at every $\lambda>0$. Amazon-Book: mixed v3/v6 execution (caption caveat applies).}
...
\begin{table}[t]
\caption{Reranking-strength sensitivity, NDCG@20, mean $\pm$ SD over five seeds. \textbf{ML-1M (matched v8):} all eight families reranked on the same fitted models, same cached scores, same player sets and $\lambda$ grid in one execution per seed; at $\lambda=0$ every family equals the base model ($0.04493$), and per-user paired contrasts per $\lambda$ are released with the run. \textbf{Amazon-Book:} still the mixed v3/v6 execution (LOO from v6, other families from v3; unequal $\lambda=0$ baselines) until the Amazon matched sweep completes. Bold column: protocol value $\lambda_{\text{attr}}=0.10$, fixed a priori and shared.}\label{tab:ablation_lambda}
\centering
\scriptsize
\resizebox{\textwidth}{!}{
