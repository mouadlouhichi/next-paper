...
| **HCCF validation artifact** | The selected primary backbone must be validated before confirmatory use. | Official-code rerun reference; validation dataset/protocol, split, preprocessing, seeds, metrics, candidate protocol, hardware, allowed implementation differences; pass criterion: mean Recall@20 and NDCG@20 over five fixed seeds each within max(5% relative, 0.005 absolute) of the official-code reference; validation logs, pass/fail decision, and fallback trigger if failed. | Missing |
...
| **HCCF validation tolerance** | Validation artifact applying the max(5% relative, 0.005 absolute) Recall@20/NDCG@20 tolerance against an official-code rerun. | Missing |
