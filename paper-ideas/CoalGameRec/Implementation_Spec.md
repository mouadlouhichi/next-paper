...
- **HitRate convention (audit #9, fixed):** under leave-one-out with one test item per user, report **HitRate@K** as the formal metric. Because each evaluated user has exactly one held-out relevant item, Recall@K is numerically identical to HitRate@K; use the label HitRate@K throughout the case study.
...
Prospective primary `shapley-mc` uses `value_mode=pairwise_logsigmoid`, `α=1.0`, `β=0.0`, and `λ_pref=0.0`. This removes the additive similarity component from the headline game. The old NDCG/ILD/preference-aware tuple is retained only as diagnostic sensitivity, because the pilot showed that embedding additive similarity in Shapley can collapse the comparison toward ordinary similarity reweighting.
...
```
v_rank,u(S_u) = mean_{i^-∈N^-_u} log σ(score_{S_u}(u,i^+_val) - score_{S_u}(u,i^-))
v_pref,u(S_u) = v_rank,u(S_u)                         [primary: λ_pref=0; no additive preference term]
```
...
**Coalition-value relevance target (fixed, leakage control):** for each user `u`, `v_pref,u(S_u)` is computed exclusively against the **validation relevance target**: the second-last temporally ordered positive interaction. The last positive interaction is reserved exclusively for final testing. Test interactions are never used to define coalition values, estimate attribution weights, tune hyperparameters, construct item vectors, select models, choose `λ_attr`, or select value-function weights. Final HitRate and NDCG are evaluated once against the frozen test target after all design and tuning decisions have been completed.

**No context or additive preference term in the prospective primary game (fixed):** the benchmark uses validation-only pairwise log-sigmoid utility as the primary coalition value. `Context(S)` is removed, `γ` is dropped, diversity is secondary/listwise, and `λ_pref=0` in the primary game. Preference/coherence variants are diagnostic sensitivity only. (If timestamps were ever used as context, the alignment score and its no-future-information property would be defined separately — they are not used here.)
...
- **Value-function terms (4.2 #10, fixed):** `v_rank,u(S_u)` is a smooth validation-only pairwise log-sigmoid utility comparing the validation positive with a fixed validation-negative set; `sim(u,i)` is used by non-game controls and diagnostic preference variants, not by the prospective primary Shapley game. Data source stated for both datasets; cross-dataset feature availability reconciled (audit #15).
- **Objective normalization (4.2 #11, fixed):** pairwise log-sigmoid values are finite negative utilities; report their empirical range and use common validation negatives across coalitions. NDCG/HitRate remain final test metrics, not the primary coalition utility.
- **Same metric as value and outcome (4.2 #12):** NDCG appears in both `v_u` and the headline outcome, and ILD/Diversity appears in both the value function and secondary diversity outcomes; disclose these as objective-aligned outcomes, not independent validation. **HitRate@20 on the held-out set** is the primary alternative outcome not directly optimized by the value function; coverage is secondary. Matched non-game objectives are run for every family.
...
- **HitRate@K** = `(1/|U|) Σ_u |relevant_u ∩ R_u@K| / |relevant_u|`. Because each evaluated user has exactly one held-out relevant item, Recall@K is numerically identical to HitRate@K. We therefore use the label **HitRate@K** throughout the case study.

Also compute **Catalogue Coverage** = `|∪_u R_u@K| / |I_eligible|` and **Intra-List Diversity (ILD)** = `(2/K(K−1)) Σ_{1≤k<l≤K} [1 − sim(i_k,i_l)]` — these are secondary; they never replace HitRate@K/NDCG@K as the headline.
...
## B.1a Primary result tables — HitRate@K and NDCG@K (filled only with realized results)

The benchmark's main-text primary deliverable is a result table reporting **HitRate@K and NDCG@K (K ∈ {5,10,20})** for the **primary family set** (`uniform`, `additive-pref`, `shapley-mc`) on the **HCCF primary backbone** and both datasets, **after** the experiments run; LightGCN is a secondary/supplementary backbone; exploratory rows (`attention`, `heuristic-pop`, and any `shapley-ai`/`myerson` kept) are clearly labelled. These tables are **populated only with realized numbers**; no predicted point values are placed here. All hypotheses below are directional only ("we test whether X > Y"); no exact figures, no bolded winners, and no mean ± std placeholders are pre-filled, so there is no risk of a placeholder being mistaken for a result.

- **Result table (main text, §7.2 of `Paper_Structure.md`):** one table per dataset for the **primary families on the HCCF primary backbone**, with columns NDCG@5/10/20 and HitRate@5/10/20; cells filled with realized mean ± std over seeds; significant differences flagged after the selected Holm correction (§A.10). Generated by `report.py` from raw outputs only.
...
|---|---|---|
| `shapley-mc` vs `uniform` (no-attribution control) | `shapley-mc` ≥ `uniform` on NDCG@20 and HitRate@20 | **medium-high** (motivated by the hypothesis that principled contribution weighting helps ranking; to be tested) |
