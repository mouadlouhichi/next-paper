...
- **BQ1.** In one interaction-player case study, does attribution-guided reweighting change held-out NDCG@K / HitRate@K relative to matched no-attribution and heuristic controls?
...
**Primary reported results — HitRate@K and NDCG@K (K ∈ {5, 10, 20}).** **Fixed hierarchy:** **Primary families** = `uniform`, `additive-pref`, `shapley-mc` (reported on the HCCF primary backbone × both datasets × all cutoffs). **Confirmatory tests** are only the four `shapley-mc` vs `uniform` HCCF contrasts at NDCG@20 and HitRate@20 across the two datasets. **Secondary controls** = `attention`, `heuristic-pop`. **Exploratory** = `shapley-ai`, `myerson`, and Study C. The headline result table reports the **primary family set on every HCCF cell**; secondary/exploratory rows are clearly labelled. `shapley-ai`/`myerson` are dropped from headline claims unless every promised cell is run (C4). Per `Implementation_Spec.md` §B.1a.
- Ranking: **NDCG@K** = `(1/|U|) Σ_u DCG_u@K / IDCG_u@K`, `DCG_u@K = Σ_{k=1..K} rel_u,k / log2(k+1)`; **HitRate@K** = `(1/|U|) Σ_u |relevant_u ∩ R_u@K| / |relevant_u|`. Under temporal leave-one-out with one test positive per user, HitRate@K equals HitRate@K; tables may use `HitRate@K (HitRate@K)` for comparability. Report realized mean ± std over seeds for every family actually run (fill results only after the experiment; per `Implementation_Spec.md` §B.1a).
...
Coverage/ILD/cost never replace HitRate@K/NDCG@K as the headline; they contextualize them.
...
7. Separately-scoped empirical case study (design, realized HitRate@K and NDCG@K results, BQ answers;
...
- [ ] **Primary realized results reported as HitRate@K and NDCG@K (K ∈ {5,10,20}) result tables** for every attribution family actually run, with mean ± std over seeds and significance flagged after correction. **No predicted values appear as results** (review 1.1/2.1); prediction register is a separate, clearly-labelled supplement linked to the external pre-registration.
