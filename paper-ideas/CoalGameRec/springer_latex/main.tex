...
\begin{table}[t]
\caption{Reranking-strength sensitivity, NDCG@20, mean $\pm$ SD over five seeds on both datasets (families and values taken from the released $\lambda$-sensitivity artifacts; 5-seed values at the protocol $\lambda=0.10$ match Table~\ref{tab:main_results}). Bold column: protocol value $\lambda_{\text{attr}}=0.10$, fixed a priori and shared. The LOO rows come from the dedicated five-seed v6 sweep (\S\ref{subsec:design_ablations} reports the tuned comparison); other families are as released in the v3 sweep artifact.}\label{tab:ablation_lambda}
\centering
\scriptsize
\resizebox{\textwidth}{!}{
