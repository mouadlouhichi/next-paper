...
- **Table 5 / Fig 3 — NDCG@K** across attribution families per dataset (primary).
- **Table 6 / Fig 4 — Recall@K** across attribution families per dataset (primary).
- **Table 7 / Fig 5 — BQ2/BQ3/BQ4** (coverage, ILD, stability, cost, cross-dataset).
...
Standalone table (matching the IJACSA/thesis convention). Include and disambiguate: `N` (player set), `S` (coalition), `v(S)`, `v_pref`, `φ_i`, `φ̂_i`, `α,β,γ,λ_pref`, `sim`, `M` (MC samples — do not also use `M` for player count), `m` (sample index), `f` (refresh frequency — do not also use `f` for the model function), `K`/`k` (cutoff/rank), `g` (communication graph), `R_u`, `U`, `I`, `rel`, the candidate set, baseline/null value, and exact vs. approximate attribution (review 4.3/6.3).
...
| Fig 5 | Scatter/heat | Secondary: coverage, ILD, stability, cost (one clear plot, not a 4-variable overload) |
| Fig 6 | Matrix | Research agenda: open problems → directions (argued from the literature, not a portfolio map) |
...
| Tab 5 | Results | NDCG@K (K=5,10,20) across attribution families, both datasets (realized, primary) |
| Tab 6 | Results | Recall@K (K=5,10,20) across attribution families, both datasets (realized, primary) |
| Tab 7 | Results | Coverage, ILD, stability, cost (BQ2/BQ3) and cross-dataset (BQ4) |
| Tab 8 | Descriptive | Dataset statistics |
| Online Resource 1 | Supplement | Complete factorial result tables + prediction register (distinct labels, not "Table A/B/C"; review C18) |
