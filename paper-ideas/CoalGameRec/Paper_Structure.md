...
## 7.1 Design
Backbones (**ported, validated HCCF — no DyHuCoG code, DOI 10.1145/3477495.3532058, official repo github.com/akaxlh/HCCF — + LightGCN**; HCCF's standard contrastive training is retained identically for all families). Attribution families with a **fixed hierarchy: Confirmatory** = `uniform` (control), `additive-pref` (matched non-game heuristic), `shapley-mc` (game-theoretic); **Secondary controls** = `attention`, `heuristic-pop`; **Exploratory** = `shapley-ai`, `myerson` (every promised cell run or dropped from headline claims). Protocol: 5 seeds, temporal leave-one-out, Adam, **full-catalogue evaluation** (no model-generated candidate pool). Datasets: MovieLens-1M, Amazon-Book (custom sample). **Primary metrics Recall@K and NDCG@K (K ∈ {5,10,20})**, secondary coverage/ILD. **Statistics (selected): confirmatory family = `shapley-mc` vs `uniform` on NDCG@20 & Recall@20 × 2 datasets on the HCCF backbone = 4 tests in one Holm family**; all other cells secondary/exploratory; per-user paired differences within seed + seed-clustered bootstrap inference (`Implementation_Spec.md` §A.10). Reference `Implementation_Spec.md` §A.2–§A.10 and §B.1a. All families share a **matched objective and tuning budget**.
...
- **Table 6 / Fig 4 — Recall@K** across confirmatory families per dataset (primary).
...
| Fig 4 | Bar | Primary results: Recall@K (K=5,10,20) across attribution families per dataset (realized) |
...
| Tab 6 | Results | Recall@K (K=5,10,20) across confirmatory families, both datasets (realized, primary) |
