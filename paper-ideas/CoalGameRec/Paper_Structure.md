...
7. Separately-scoped empirical case study (planned for external preregistration)
   ...
   7.2 Results — Recall@K/HitRate@K and NDCG@K (K ∈ {5,10,20}) realized tables (BQ1) +
       coverage/ILD/cost (BQ2–BQ4)
   7.3 The case study's relation to one taxonomy slice (not the whole taxonomy)
...
4. **A separately-scoped empirical case study (planned for external preregistration)** instantiating the interaction-player / ranking-utility slice of the taxonomy, reporting **Recall@K/HitRate@K and NDCG@K (K ∈ {5,10,20}) as the primary realized result tables** (BQ1–BQ4) with matched controls. It is a supporting study, not the basis for the survey's field-wide claims (review 2.5).
...
## 1.4 Relationship to prior surveys (positioning table)
**Table 1.** Rows: representative prior surveys (general Shapley-XAI; GNN explanation; game-theoretic recommender single papers) and known near-neighbors (ShaRP, data valuation for RS, TU-bandit creator-incentive games, VLDB Shapley-in-data-analytics). Columns: *scope = recommendation ranking*, *coalitional solution concepts (beyond SHAP feature attribution)*, *taxonomy*, *graph/hypergraph focus*, *empirical grounding*. The novelty claim is that this combination is not covered by any single prior work — **to be confirmed by the systematic search**, not asserted (review 1.6/2.2).
...
## 3.3 PRISMA 2020 flow and required quality assessment
**Figure 1.** PRISMA 2020 flow (records identified → screened → eligible → included) with counts, plus PRISMA-S search reporting. **Quality/risk-of-bias assessment is required (not optional)** — score each included study on the §4.5 rubric of `spec.md` (review 2.2). Record screening counts, disagreements, and agreement measure; report included core vs. adjacent works separately.
...
## 4.6 The taxonomy tuple and worked examples
...
## 7.1 Design
Backbones (**HCCF selected as the primary backbone; a port will be pinned and validated before preregistration — no DyHuCoG code, DOI 10.1145/3477495.3532058, official repo github.com/akaxlh/HCCF — + secondary LightGCN**; HCCF's standard contrastive training is retained identically for all families). Attribution families with a **fixed hierarchy: Confirmatory** = `uniform` (control), `additive-pref` (matched non-game heuristic), `shapley-mc` (game-theoretic); **Secondary controls** = `attention`, `heuristic-pop`; **Exploratory** = `shapley-ai`, `myerson` (every promised cell run or dropped from headline claims). Protocol: 5 seeds, temporal leave-one-out, Adam, **full-catalogue evaluation** (no model-generated candidate pool). Datasets: MovieLens-1M, Amazon-Book (custom sample). **Primary metrics Recall@K/HitRate@K and NDCG@K (K ∈ {5,10,20})**, secondary coverage/ILD. **Statistics (selected): confirmatory family = `shapley-mc` vs `uniform` on NDCG@20 & Recall@20/HitRate@20 × 2 datasets on the HCCF backbone = 4 tests in one Holm family**; all other cells secondary/exploratory; per-user paired differences within seed + seed-clustered bootstrap inference (`Implementation_Spec.md` §A.10). Reference `Implementation_Spec.md` §A.2–§A.10 and §B.1a. All families share a **matched objective and tuning budget**.

## 7.2 Results
The primary realized results are the **Recall@K/HitRate@K and NDCG@K (K ∈ {5,10,20}) tables** for the **confirmatory families** (`uniform`, `additive-pref`, `shapley-mc`) on the **HCCF primary backbone, both datasets**, reported as mean ± std over seeds with significance flagged after the selected Holm correction (§A.10). **These tables are filled only with realized numbers; no predicted values appear as results** (review 1.1/2.1). Present per the figure/table map and report the measured ordering neutrally — **do not assume or instruct that `shapley-mc` ranks 1st**; report whichever ordering occurs (review 2.1):
- **Table 5 / Fig 3 — NDCG@K** across confirmatory families per dataset (primary).
- **Table 6 / Fig 4 — Recall@K/HitRate@K** across confirmatory families per dataset (primary).
...
## 7.3 The case study's relation to one taxonomy slice
...
| Fig 4 | Bar | Primary results: Recall@K/HitRate@K (K=5,10,20) across attribution families per dataset (realized) |
| Fig 5 | Bar | Secondary: coverage and ILD across confirmatory families (BQ2) |
...
| Tab 5 | Results | NDCG@K (K=5,10,20) across confirmatory families, both datasets (realized, primary) |
| Tab 6 | Results | Recall@K/HitRate@K (K=5,10,20) across confirmatory families, both datasets (realized, primary) |
