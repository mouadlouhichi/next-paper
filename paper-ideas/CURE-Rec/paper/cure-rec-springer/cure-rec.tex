...
Figure~\ref{fig:workflow} gives the complete workflow as one narrow vertical path, following the presentation style of the reference paper. The first seven boxes define the controlled intervention-game path. The final dashed box is deliberately separated as an implementation audit: MovieLens ranking results validate the external models and evaluator but do not enter the causal coalition value.
...
\begin{tikzpicture}[
  font=\footnotesize,
  node distance=3.7mm,
  stage/.style={draw=black, rounded corners=1pt, align=center, minimum height=8.2mm, minimum width=71mm, fill=white, line width=.55pt},
  audit/.style={draw=black, dashed, rounded corners=1pt, align=center, minimum height=8.2mm, minimum width=71mm, fill=white, line width=.55pt},
  arr/.style={-{Latex[length=1.7mm]}, draw=black, line width=.6pt},
  note/.style={-{Latex[length=1.7mm]}, draw=black, dashed, line width=.55pt}
]
\node[stage] (state) {Base policy $\BasePolicy$ and observable platform state};
\node[stage, below=of state] (players) {Six feasible intervention players};
\node[stage, below=of players] (coalitions) {Canonical composition and all $2^6=64$ intervention coalitions};
\node[stage, below=of coalitions] (rollouts) {Shared-seed scenario rollouts over satisfaction, retention, fatigue, relevance, and provider exposure};
\node[stage, below=of rollouts] (values) {Scenario coalition values and feasibility diagnostics};
\node[stage, below=of values] (attribution) {Exact Shapley regions and pairwise interaction analysis};
\node[stage, below=of attribution] (planner) {Maximin feasibility planner: improve, repair, abstain, or no feasible portfolio};
\node[stage, below=of planner] (output) {Selected portfolio, explanation card, trajectories, tables, and reproducibility manifest};
\node[audit, below=of output] (external) {Separate external-ranking audit: shared MovieLens candidates, validation selection, frozen BPR/SASRec tests};

\draw[arr] (state) -- (players);
\draw[arr] (players) -- (coalitions);
\draw[arr] (coalitions) -- (rollouts);
\draw[arr] (rollouts) -- (values);
\draw[arr] (values) -- (attribution);
\draw[arr] (attribution) -- (planner);
\draw[arr] (planner) -- (output);
\draw[note] (output) -- (external);
\node[draw=black, dashed, rounded corners=1pt, fit=(coalitions)(rollouts)(values)(attribution), inner sep=3pt,
 label={[font=\scriptsize]left:exact cooperative intervention-game layer}] {};
\end{tikzpicture}
\caption{Workflow of the proposed \CURE{} framework. One vertical chain defines, evaluates, attributes, and selects a policy-intervention portfolio. The final dashed box is an external ranking audit and is not used to infer causal policy effects from MovieLens.}
\label{fig:workflow}
\end{figure}
...
\begin{axis}[
  ...
  ylabel={Jaccard similarity to oracle portfolio},
  symbolic x coords={Add,Comp,Red,Ant,Short,Long,PBal,PRep,Mis},
  xtick=data,
  xticklabels={additive,complementary,redundant,antagonistic,short,long,balance repair,repeat repair,misspecified},
  x tick label style={rotate=45,anchor=east,font=\scriptsize},
  ymajorgrids=true, grid style={dashed,black!35},
  nodes near coords, every node near coord/.append style={font=\scriptsize},
  enlarge x limits=0.04
]
\addplot+[draw=black,fill=black!22] coordinates {(Add,1) (Comp,1) (Red,1) (Ant,1) (Short,1) (Long,1) (PBal,1) (PRep,1) (Mis,0)};
\end{axis}
\end{tikzpicture}
\caption{Controlled oracle recovery. Eight regimes recover the oracle portfolio exactly. The misspecified ambiguity regime is the intended failure: the estimated game selects repeat cap while the oracle selects exploration.}
\label{fig:regimes}
\end{figure}
...
\begin{axis}[
  ...
  ybar, bar width=7pt, ymin=0, ymax=0.42,
  ylabel={Mean robust lower improvement},
  symbolic x coords={Base,R2,R4,H8,H16,P24,P32,N03,N06},
  xtick=data,
  xticklabels={base,$r{=}2$,$r{=}4$,$T{=}8$,$T{=}16$,$p{=}0.24$,$p{=}0.32$,$n{=}0.03$,$n{=}0.06$},
  x tick label style={font=\scriptsize},
  ymajorgrids=true, grid style={dashed,black!35},
  nodes near coords, every node near coord/.append style={font=\scriptsize,rotate=90,anchor=west},
  enlarge x limits=0.04
]
\addplot+[draw=black,fill=black!22] coordinates {
 (Base,0.29488) (R2,0.20206) (R4,0.21226) (H8,0.14397) (H16,0.37785)
 (P24,0.27997) (P32,0.29488) (N03,0.29502) (N06,0.29513)};
\end{axis}
\end{tikzpicture}
\caption{One-at-a-time sensitivity of mean robust lower improvement. $r$ is the repeated-exposure threshold, $T$ the horizon, $p$ the provider-disparity threshold, and $n$ the delayed novelty-preference drift. All bars summarize five independent seeds; numerical values are reported in Table~\ref{tab:oat}.}
\label{fig:oat}
\end{figure}
