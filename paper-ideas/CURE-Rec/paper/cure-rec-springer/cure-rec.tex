...
\title[CURE-Rec]{CURE-Rec: Exact Cooperative Intervention Games for Robust Long-Term Recommendation}
...
Collaborative filtering and sequential recommendation are usually introduced as supervised ranking problems, from matrix factorization and Bayesian personalized ranking \cite{rendle2009bpr} to self-attentive sequence models \cite{kang2018sasrec} and graph-based methods \cite{he2020lightgcn}. These models are powerful predictors, but prediction quality does not by itself decide whether a platform should increase exploration, reduce repetition, or rebalance exposure. A recommendation is an action that changes the observations on which future training and ranking depend. This closed loop motivates causal recommender work and counterfactual learning-to-rank \cite{swaminathan2015counterfactual,joachims2017unbiased,wang2021causal}.

The available evidence often limits what can be inferred. Observational ratings typically record a response but not the alternatives shown, the rank at which they were shown, the logging probability, or the contemporaneous candidate set. Treating such records as complete randomized policy logs conflates preference, exposure, and selection. Work on recommendation as treatment makes this distinction explicit \cite{schnabel2016recommendations}. \CURE{} follows that discipline: the causal intervention game is tested in an oracle benchmark; the MovieLens experiment remains a separate chronological-ranking robustness evaluation.

\subsection{Diversity, novelty, repetition, and provider exposure}

A long literature improves recommendation lists through diversity, novelty, long-tail exposure, and popularity-aware reranking \cite{ziegler2005improving,vargas2011rank,abdollahpouri2019popularity}. These contributions usually define one target metric or one re-ranking objective. The operational challenge is that their transformations can collide. A diversity reranker can alter the positions available to a long-tail slot; a novelty slot and a tail slot may propose the same item; and provider balancing can reduce disparity while changing relevance or fatigue. The present work does not replace those objectives. It offers a framework for assessing the exact value of their feasible combinations under a shared platform utility and explicit constraints.
...
\label{eq:utility}
\end{equation}
The reported improvement is $\Delta\Utility_m(S)=\Utility_m(S)-\Utility_m(\varnothing)$. We retain utility, satisfaction, retention, fatigue, relevance, catalogue coverage, and provider disparity separately; a scalar utility is useful for comparison but must not hide constraint violations.
...
Figure~\ref{fig:workflow} gives the complete workflow. The upper path constructs a reproducible sequential policy environment and evaluates all coalitions under shared random state within each seed. The middle path computes attribution and interactions from the exact game. The lower path uses feasibility constraints and robust improvement to choose a portfolio. A separate, explicitly labelled external-evaluation path checks that the recommender implementations and shared ranking evaluator behave correctly on MovieLens-1M; it does not feed observational ratings into the causal game.

\begin{figure*}[t]
\centering
\begin{tikzpicture}[
  font=\small,
  node distance=0.90cm and 0.95cm,
  input/.style={draw, rounded corners=2pt, align=center, minimum height=9mm, minimum width=27mm, fill=blue!7},
  game/.style={draw, rounded corners=2pt, align=center, minimum height=9mm, minimum width=30mm, fill=orange!14},
  decision/.style={draw, rounded corners=2pt, align=center, minimum height=9mm, minimum width=31mm, fill=green!12},
  audit/.style={draw, rounded corners=2pt, align=center, minimum height=9mm, minimum width=30mm, fill=purple!9},
  arr/.style={-{Latex[length=2mm]}, thick, draw=black!75},
  dash/.style={-{Latex[length=2mm]}, thick, dashed, draw=black!65}
]
\node[input] (policy) {Base policy\\and public state};
\node[input, right=of policy] (library) {Six bounded\\intervention players};
\node[game, right=of library] (coalitions) {Exact coalitions\\$2^6=64$};
\node[game, right=of coalitions] (rollout) {Scenario rollouts\\utility and constraints};
\node[game, below=of coalitions] (credit) {Exact Shapley regions\\and interactions};
\node[decision, right=of credit] (planner) {Robust planner\\improve, repair, or abstain};
\node[decision, right=of planner] (card) {Portfolio decision\\and explanation card};
\node[audit, below=of policy] (external) {External ranking protocol\\MovieLens split and candidates};
\node[audit, right=of external] (models) {Popularity, BPR, SASRec\\validation selection};
\node[audit, right=of models] (checks) {Audit: seen-item, target,\\candidate, checkpoint checks};

\draw[arr] (policy) -- (library);
\draw[arr] (library) -- (coalitions);
\draw[arr] (coalitions) -- (rollout);
\draw[arr] (rollout) -- (credit);
\draw[arr] (credit) -- (planner);
\draw[arr] (planner) -- (card);
\draw[arr] (external) -- (models);
\draw[arr] (models) -- (checks);
\draw[dash] (policy.south) -- (external.north);
\draw[dash] (checks.north) -- ++(0,0.42) -| (card.south);
\node[draw, dashed, rounded corners=2pt, fit=(coalitions)(rollout)(credit), inner sep=5pt,
 label={[font=\small]below:exact cooperative intervention-game layer}] {};
\end{tikzpicture}
\caption{Workflow of the proposed \CURE{} framework. The causal-oracle and behavioural claims are produced by the upper and middle paths in the disclosed sequential benchmark. The lower external path audits chronological ranking implementations on MovieLens-1M. It is deliberately not used to assert causal policy effects.}
\label{fig:workflow}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/figure_02_controlled_regimes.png}
\caption{Controlled-regime recovery. The figure is generated from the archived oracle suite. Eight of nine regimes recover the oracle portfolio; the misspecified ambiguity regime is the intended failure case.}
\label{fig:regimes}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/figure_03_oat_sensitivity.png}
\caption{One-at-a-time behavioural sensitivity of robust lower improvement. This generated figure is based on 15 predeclared configurations and five independent seeds per configuration.}
\label{fig:oat}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/figure_04_lhs_sensitivity.png}
\caption{Joint Latin-hypercube sensitivity of robust lower improvement. Colour encodes repair rate. The figure reports a baseline plus 24 predeclared joint configurations, each evaluated over five seeds.}
\label{fig:lhs}
\end{figure}
...
\label{eq:candidates}
\end{equation}
A held-out target absent from the training catalogue is counted as cold rather than silently removed. The final evaluations report 1,000 warm-target users, one cold test target, mean candidate count 3,436.958, and candidate counts from 2,739 to 3,521.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/figure_05_lhs_stability.png}
\caption{Decision stability across the 25 joint calibration configurations. The full seed-level portfolio distribution is retained in the archive; this figure summarizes configuration-level stability, repair rate, base feasibility, and repeat-cap selection rate.}
...
\begin{tikzpicture}
\begin{axis}[
 width=0.98\linewidth, height=6.3cm,
 ybar, bar width=13pt, ymin=0, ymax=0.17,
 ylabel={Metric value},
 symbolic x coords={Popularity,BPR-MF,SASRec},
 xtick=data, legend style={at={(0.5,1.06)},anchor=south,legend columns=-1},
 nodes near coords, every node near coord/.append style={font=\scriptsize},
 ymajorgrids=true, grid style=dashed
]
\addplot coordinates {(Popularity,0.04900) (BPR-MF,0.08800) (SASRec,0.14520)};
\addplot coordinates {(Popularity,0.02552) (BPR-MF,0.04384) (SASRec,0.07424)};
\legend{$\Recall$, $\NDCG$}
\end{axis}
\end{tikzpicture}
\caption{External MovieLens-1M ranking results. Bars show five-seed means for BPR-MF and SASRec and the deterministic popularity value under the shared warm-item protocol.}
\label{fig:external}
\end{figure}
