...
The pre-specified user-level analysis has since been executed: the frozen final configurations were re-trained for seeds 42--46, one hit/NDCG row per evaluated warm user was exported, and paired user-level inference was computed (Table~\ref{tab:ml1m-userlevel}). With the user as the inferential unit ($n=1{,}000$), every comparison is significant after Holm correction across the model$\times$metric family: exact McNemar tests on the per-user hit indicators give $p\leq1.5\times10^{-4}$ (seed 42) and $p\leq1.5\times10^{-10}$ on the five-seed per-user means; paired sign-flip permutation tests on NDCG give $p\leq0.0014$; and all paired user bootstrap confidence intervals for the mean differences exclude zero. Effect sizes are small-to-moderate ($d_z$ between $0.099$ and $0.280$), so the claim is a statistically supported improvement under the audited warm-item protocol, not a large-effect claim. These remain chronological-ranking results: they do not estimate causal policy effects and do not validate the \CURE{} portfolio-selection layer.

\begin{table}[t]
\caption{MovieLens-1M user-level paired inference against popularity ($n=1{,}000$ warm users; five-seed per-user means). CI: paired user bootstrap, 95\%. $p$: Holm-adjusted exact sign test (McNemar) on per-user hits.}\label{tab:ml1m-userlevel}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\resizebox{\linewidth}{!}{%
...
The reproducibility snapshot is intentionally small enough to inspect. It contains the external data-analysis manifest and evaluator tables; BPR staged-search and final-audit files; SASRec final configuration, evaluation audit, pairwise audit, loss, validation, seed metrics, paired metrics, and summary; controlled-regime manifest, selections, attributions, interactions, and figure; and OAT/LHS manifests, configurations, summaries, seed decisions, attributions, interactions, and figures. It omits raw MovieLens data and large model checkpoints. The archive manifest explains how to regenerate those omitted artifacts from the source repository. The revision experiments reported in Sections~\ref{sec:divergent-selectors},~\ref{sec:objective-sensitivity}, and~\ref{sec:semireal} are archived under \texttt{code/results/reviewer\_phase\_assets/}---\texttt{divergent\_selector\_holdout/}, \texttt{objective\_constraint\_sweeps/}, \texttt{semireal\_integration/}, \texttt{integrated\_scalability/}, and \texttt{movielens\_1m\_paired/} (per-user metrics plus paired user-level inference)---with their own manifests and a checksum file (\texttt{SHA256SUMS.txt}), following the same evidence discipline as the primary snapshot. Remaining executions that require network downloads or long compute (the integrated $n=10$ game and a second-domain ranking audit) are gathered in \texttt{code/notebooks/13\_reviewer\_closure\_runs.ipynb}; the notebook verifies already-archived outputs where they exist and emits manuscript-ready tables for the rest.
