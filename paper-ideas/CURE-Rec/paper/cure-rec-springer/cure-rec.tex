...
For scenario $m$ and coalition $S$, let $\Utility_m(S)$ be the net discounted cohort utility after intervention cost. We use discounted satisfaction and retention, less fatigue, and subtract coalition cost:
\begin{equation}
 \Utility_m(S)=\sum_{t=0}^{T-1}\gamma^t\left(w_s\,\mathrm{Sat}_{m,t}(S)+w_r\,\mathrm{Ret}_{m,t}(S)-w_f\,\mathrm{Fat}_{m,t}(S)\right)-w_c\,c(S).
\label{eq:utility}
\end{equation}
The reported improvement is $\Delta\Utility_m(S)=\Utility_m(S)-\Utility_m(\varnothing)$. In the pre-specified full configuration, $\gamma=0.97$, $(w_s,w_r,w_f,w_c)=(0.70,0.30,0.35,1.00)$, and the components are bounded cohort means in $[0,1]$. Satisfaction is the discounted mean of the response-level satisfaction probability, fatigue is the discounted mean user fatigue, and retention is $\mathrm{clip}(1-\mathrm{fatigue}+0.25\,\mathrm{satisfaction},0,1)$. Thus the utility is a normalized simulator scale rather than a currency or a user-percentage quantity. We retain utility, satisfaction, retention, fatigue, relevance, catalogue coverage, and provider disparity separately; a scalar utility is useful for comparison but must not hide constraint violations.
...
\end{equation}
Here $r(S)$ is the lower scenario relevance difference, $d(S)$ the upper scenario provider disparity, and $f(S)$ the upper scenario fatigue. We compute provider disparity as the Gini coefficient of nonnegative provider-exposure counts $x_1,\ldots,x_p$: $G(x)=\frac{\sum_{a=1}^{p}\sum_{b=1}^{p}|x_a-x_b|}{2p\sum_{a=1}^{p}x_a}$, with $G(x)=0$ when total exposure is zero. The implementation stores each scalar, rather than only the indicator, because a failed coalition must be explainable: it matters whether the failure arose from budget, relevance, fatigue, provider disparity, or several constraints at once.
...
For a fixed seed and scenario, every coalition begins from the same initial environment state and random-number construction. This common-random-number design aligns paired coalition differences under the same synthetic cohort draw and can reduce their variance when the resulting outcomes are positively correlated; such a reduction is not guaranteed. Seeds are independent across repetitions. Each run writes a manifest, JSONL events, coalition values, attribution regions, interaction regions, decision data, trajectories, and generated tables and figures.

Common random numbers do not guarantee a variance reduction; they align within-seed shocks so that paired differences can be measured. In the dedicated stochastic click-feedback diagnostic, the observed variance ratios were approximately one. A coalition can still reduce utility, violate relevance, or fail a provider constraint; the grand coalition is in fact harmful in the observed behavioural scenarios. The design merely ensures that a comparison between two coalitions is not dominated by an avoidable difference in simulated initial state. We preserve independent seeds because a common-random-number result from one seed is not a stability claim.
...
The grand coalition is budget-infeasible ($c(\Players)=0.49>B=0.35$) and has negative robust-game value. Table~\ref{tab:selector-baselines} reports its zero-valued infeasible-fallback diagnostic separately from that raw negative value; it is not a selectable portfolio. This rejects an additive ``more interventions is always better'' interpretation. Interaction analysis explains why: several injection and reranking combinations introduce relevance, cost, or fatigue trade-offs that a single-intervention ablation cannot expose.
...
Grand-coalition diagnostic (infeasible fallback) & 0.000000 & 0.000000 & -0.294584 & 0.000002\\
\bottomrule
\end{tabular}}
\end{table*}

The primary configuration produces a deliberately uninformative tie among several selectors because repeat cap is dominant. We therefore ran a second, pre-specified held-out comparison on three constraint-frontier configurations. Selection used seeds 60--62 and frozen portfolios were evaluated on disjoint seeds 260--269. In the strict-provider and tight-relevance settings, best-singleton and robust-Shapley-1 were below the exact selector (for example, mean held-out lower improvements of $-0.07293$ versus $-0.02369$ in one frontier run), whereas nominal and some greedy rules remained tied with the exact selector. In the relaxed-provider setting, the selectors again largely agreed on repeat cap. These results show that the decision rule can matter when feasibility creates nontrivial alternatives, but do not support universal superiority claims.

\begin{table}[t]
\caption{Held-out selector comparisons under selected constraint-frontier settings. Values are means over disjoint evaluation seeds; only valid portfolio selectors are interpreted, while grand-coalition rows remain diagnostics.}
\label{tab:divergent-selectors}
\centering
\scriptsize
\begin{tabular}{@{}llrrr@{}}
\toprule
Frontier & Selector & Lower improvement & Feasible rate & $\Delta$ vs. CURE\\
\midrule
Strict provider & CURE exact maximin & $-0.02369$ & 0.50 & 0.00000\\
Strict provider & Best singleton & $-0.07293$ & 0.67 & $-0.04924$\\
Strict provider & Robust Shapley-1 & $-0.07293$ & 0.67 & $-0.04924$\\
Tight relevance & CURE exact maximin & $-0.02369$ & 0.50 & 0.00000\\
Tight relevance & Best singleton & $-0.07293$ & 0.67 & $-0.04924$\\
Relaxed provider & CURE exact maximin & 0.29320 & 1.00 & 0.00000\\
Relaxed provider & Best singleton & 0.29320 & 1.00 & 0.00000\\
\bottomrule
\end{tabular}
\end{table}

The selector comparison is intentionally not a manufactured superiority claim. Exact CURE, singleton, robust-Shapley, greedy, and nominal selectors all choose repeat cap for all selection seeds in this configuration, so they are tied on the held-out evaluation. The exact framework is markedly better than random feasible selection; the grand coalition is categorically budget-infeasible under the reported cost and budget values and is reported only as a diagnostic. The value of CURE-Rec in this regime is therefore not that it beats every heuristic, but that it identifies and explains the same simple intervention while retaining principled repair, constraint, and interaction diagnostics for settings in which selectors diverge.
...
\begin{center}
\refstepcounter{table}\label{tab:external}
\textbf{Table \thetable.} MovieLens-1M fixed-configuration five-seed ranking results under shared warm-item candidates. Mean $\pm$ sample standard deviation.\par\smallskip
\scriptsize
...
We conducted the additional reviewer-revision checks without changing the pre-specified primary CURE-Sim configuration. The held-out selector study selected on seeds 42--46 and evaluated frozen portfolios on disjoint seeds 200--219. Because the full configuration made the base policy infeasible, the decision was correctly interpreted in repair mode: repeat-cap was selected in the held-out study with a robust lower improvement of $0.29235$ and an upper improvement of $0.31500$. This result is a simulator-conditional held-out decision result, not evidence of real-world causal policy performance.
...
We also evaluated a distinct 6/8/10-player attribution-approximation benchmark with coalition counts 64, 256, and 1024 and sampled budgets 32--2048. The added players were session-length cap, freshness quota, provider cooldown, and category-coverage quota. Because these additional operators are not yet integrated into the primary CURE-Sim policy semantics, this result is an arithmetic scalability stress test rather than a causal CURE-Sim scalability claim.

Finally, MovieLens-25M was downloaded automatically and evaluated under a separate chronological warm-item protocol. For 1,000 paired users, the NumPy BPR fallback was below popularity by $-0.046$ Hit Rate (95\% bootstrap CI $[-0.060,-0.033]$, $d_z=-0.219$) and by $-0.02040$ NDCG (95\% CI $[-0.02688,-0.01437]$, $d_z=-0.203$); Holm-adjusted p-values were $5.68\times10^{-14}$. This is descriptive ranking evidence for the tested fallback and dataset, not evidence that BPR is generally inferior and not causal policy evidence. The second dataset therefore supplies a robustness check with an unfavorable outcome rather than a selectively favorable replication.
...
The OAT study has 75 decisions. Repeat cap is included in all 75 and is the exact portfolio in 74. The joint LHS study has 125 decisions. Its seed-level portfolios are: repeat cap alone (89), repeat cap plus tail slot (12), repeat cap plus exploration slot (8), provider balance alone (6), abstention (4), repeat cap plus diversity (2), repeat cap plus provider balance (2), repeat cap plus exploration plus tail (1), and tail slot alone (1). The 125 decisions comprise 83 repair-mode cases and 42 improvement-mode cases; four of the 42 improvement-mode cases abstain. These are status counts, not mutually exclusive portfolio labels. These counts are reported to make the joint-switching result auditable rather than summarized only by a modal portfolio.
...
\begin{align}
 ...
 Q_{j,t+1}&=Q_{j,t}+\beta_m\rho\,\sum_{u\in\mathcal U}\mathbf{1}\{j\in L_{u,t}\},\\
