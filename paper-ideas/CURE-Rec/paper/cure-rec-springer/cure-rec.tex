...
\begin{equation}
 A_S^{\star}(X_t)=\arg\max_A\sum_{(i,j)\in A}\widetilde b_i(j)
\quad\text{s.t.}\quad |A|\leq q,\quad |\{j:(i,j)\in A\}|=|A|,
\label{eq:assignment}
\end{equation}
where at most one item is assigned to each active intervention and an item may not be assigned twice. The assignment is solved by exhaustive enumeration because there are at most three active injection players and a small proposal list. A stable item/intervention tie rule makes $A_S^{\star}$ deterministic.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{figures/figure_01_workflow.pdf}
\caption{Vertical workflow of the proposed \CURE{} framework. The black-and-white workflow is generated from the same manuscript figure script as the quantitative figures. The dashed final box is an external ranking audit and is not used to infer causal policy effects from MovieLens.}
\label{fig:workflow}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/figure_02_controlled_recovery.pdf}
\caption{Controlled oracle recovery across the nine declared regimes. Eight regimes recover the oracle portfolio exactly; the misspecified ambiguity regime is the intentional failure case.}
\label{fig:regimes}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/figure_04_lhs_attribution.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.90\linewidth]{figures/figure_05_attribution_decision.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/figure_06_external_ranking.pdf}
