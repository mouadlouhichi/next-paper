...
\begin{tikzpicture}[
  font=\footnotesize,
  node distance=4.3mm,
  stage/.style={draw=black, rounded corners=1.5pt, align=center, minimum height=8.5mm, minimum width=64mm, fill=white, line width=.55pt},
  side/.style={draw=black, rounded corners=1.5pt, align=center, minimum height=8.5mm, minimum width=39mm, fill=white, line width=.5pt},
  arr/.style={-{Latex[length=1.8mm]}, draw=black, line width=.6pt},
  note/.style={-{Latex[length=1.6mm]}, draw=black, dashed, line width=.5pt}
]
\node[stage] (state) {Base policy $\BasePolicy$ and observable platform state};
\node[stage, below=of state] (players) {Six feasible players: repetition, exploration, tail, diversity, novelty, provider balance};
\node[stage, below=of players] (coalitions) {Canonical composition and exact coalition set $S\subseteq\Players$, $|\Players|=6};
\node[stage, below=of coalitions] (rollouts) {Shared-seed scenario rollouts: satisfaction, retention, fatigue, relevance, provider exposure};
\node[stage, below=of rollouts] (game) {Exact coalition values, Shapley regions, and pairwise interactions};
\node[stage, below=of game] (decision) {Feasibility filter and maximin planner: improve, repair, abstain, or no feasible portfolio};
\node[stage, below=of decision] (output) {Selected portfolio, explanation card, audit tables, trajectories, and reproducibility manifest};

\node[side, right=11mm of rollouts] (external) {Separate external path\\MovieLens chronological ranking\\shared candidates and audit};
\node[side, below=of external] (models) {Popularity, BPR, SASRec\\validation selection then frozen test};

\draw[arr] (state) -- (players);
\draw[arr] (players) -- (coalitions);
\draw[arr] (coalitions) -- (rollouts);
\draw[arr] (rollouts) -- (game);
\draw[arr] (game) -- (decision);
\draw[arr] (decision) -- (output);
\draw[note] (rollouts.east) -- (external.west);
\draw[note] (external) -- (models);
\draw[note] (models.west) -| (output.east);
\node[draw=black, dashed, rounded corners=1.5pt, fit=(coalitions)(rollouts)(game), inner sep=3.5pt,
 label={[font=\scriptsize]left:exact intervention-game layer}] {};
\end{tikzpicture}
\caption{Vertical workflow of the proposed \CURE{} framework. The central path defines and evaluates an exact cooperative game over policy interventions, then selects a constrained robust portfolio. The dashed external path validates ranking implementations on MovieLens-1M but is not used to infer causal policy effects.}
\label{fig:workflow}
\end{figure}
