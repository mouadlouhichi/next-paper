...
def permutation_precision(paired: pd.DataFrame, d: pd.DataFrame, draws: int = 50_000) -> list[dict]:
    ...
    The question is whether a printed $p$ reflects censoring or the actual
...
def candidate_redraw_tables() -> tuple[list[str], dict]:
    ...
    L = table(
        ...
        "Min--max redraw mean",
        "Independent candidate-set resamples. Every redraw draws the "
...
def compute_matched_tables() -> tuple[list[str], dict]:
    ...
    L = table(
        ...
        "Compute-matched budget-response curves. Each row is one budget "
...
def hardware_tables() -> tuple[list[str], dict]:
    ...
    L = table(
        ...
        "with the audit runs. Times are medians over the declared "
...
def render(flow, sens, mult, mult_stats, factorial, stud, mcp, prec) -> str:
    L: list[str] = [
        ...
        "% Inference audits and construct-validity ablations, computed from the frozen release matrices.",
        "",
    ]
    ...
    L += table(
        ...
        "$2\\times2\\times2$ design is reported for the nonlinear "
    ...
    L += table(
        ...
        "validation this cross-check exists to provide; where they differ, the studentized "
    ...
    L += table(
        ...
        "the propagation beyond the rank-agreement "
