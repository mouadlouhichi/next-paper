...
for experiment, _extra, why, _est in PLAN:
    ...
    scale_note = ("fixed-cost job" if experiment in FIXED_COST
                  else f"~{per_1000:5.0f} min per 1000 users at M={PERM}")
...
if any(v != "ok" for v in pilot.values()):
    ...
    total = sum((MEASURED[e] if e in FIXED_COST else MEASURED[e] * USERS[d] * SCALE / 1000.0)
...
for experiment, dataset, args, target, est in jobs():
    ...
    if experiment in measured:                       # measured on this machine
        minutes = (measured[experiment] if experiment in FIXED_COST
...
_rc, _out = sh(["python3", str(SCRIPTS / "make_review9_stats.py")], cwd=CODE)
_lines = [l for l in _out.splitlines()
          if l.startswith("wrote") or "ablation:" in l or "report" in l.lower()]
print("\n".join(_lines[-10:]) if _lines else _out[-500:])

print("\n== 2. re-freeze the result manifest ==")
rc, out = sh(["python3", str(SCRIPTS / "make_result_manifest.py")], cwd=CODE)
print(out[-600:])
