...
\title[ActionShap]{ActionShap: An Offline Protocol for Evaluating Recommendation Attributions under Bounded Profile Interventions}
...
\keywords{Explainable recommender systems, Attribution evaluation, Intervention evaluation, Counterfactual evaluation, Recommendation faithfulness, Bounded profile intervention, Shapley values}
...
Explainable recommendation includes feature, item, path, attention, sentiment, and natural-language rationales. Surveys emphasize that these forms serve different objectives, including user persuasion, model fidelity, transparency, and preference communication \cite{zhang2020survey}. Latent-factor explanations expose item or feature contributions, while post-hoc methods apply model-agnostic surrogates such as LIME and SHAP \cite{peake2018,nobrega2019,ribeiro2016,lundberg2017}. Sentiment- and phrase-level models construct explanations during recommendation rather than evaluating a post-hoc attribution \cite{zhang2014}. Fidelity-oriented methods now train the explanation mechanism itself against path-integration objectives across multiple recommender architectures and explicitly warn that unrealistic attribution baselines distort sparse-recommendation audits \cite{barkan2026spinrec}; counterfactual reasoning has likewise been used to explain path-based recommendations beyond attention scores \cite{li2024tkde}. ActionShap addresses a complementary question: whether an explanation predicts the outcome of a declared feasible change.
...
Deletion and insertion tests measure model dependence but can be sensitive to the baseline and perturbation path. Sanity checks show that some attribution maps can appear plausible while failing model or data randomization tests \cite{adebayo2018}. Perturbation-based explanations also raise the question of whether the perturbation remains within a meaningful data manifold \cite{hooker2019}. Counterfactual recommendation and recourse methods instead search for profile or graph changes that alter a ranking or decision \cite{ghazimatin2020,tran2021,tan2021,yao2022,baklanov2024,barkan2024,gurevitch2025,mohammadi2025,nguyen2026,wachter2017,karimi2021}, including minimal-perturbation counterfactuals generated by guided denoising diffusion \cite{mohammadi2026sigir} and plausibility-constrained counterfactuals validated with user studies \cite{cerny2025plausible}; a parallel line argues that automated counterfactual metrics alone do not exhaust explanation evaluation and that human-centric criteria must be included \cite{domnich2025unifying}. Dedicated benchmarks for recommendation explanations, including LLM-powered automatic evaluation \cite{li2025alert}, show that explanation evaluation itself is becoming a standalone scalable task; path-integration attribution \cite{sundararajan2017axiomatic} provides the continuous-path baseline that our bounded-weight semantics approximate discretely. ActionShap does not generate causal recourse. It evaluates an attribution under a fixed intervention policy and makes the policy, cost, authority, and offline limitations explicit.

\subsection{Evaluation protocols for recommender explanations}
A distinct literature evaluates recommendation explanations rather than generating them. Deletion-style audits dominate, but recent protocols target other semantics: rank-based attribution fidelity \cite{chowdhury2024rankshap,pliatsika2025sharpl}, event-deletion for sequential outputs \cite{bento2021timeshap}, contrastive explanation benchmarks \cite{mousavi2023clime}, session-model explanation evaluation \cite{bruun2025session}, and LLM-powered automatic explanation assessment \cite{li2025alert}. Two findings are directly relevant to ActionShap's design. First, recommender performance itself changes explainer rankings, so explainer evaluation must be gated on recommendation quality \cite{mohammadi2024recsys}; ActionShap therefore audits model quality against popularity before any attribution result is interpreted (Section~\ref{sec:robustness} shows why this gate matters). Second, fidelity measured along one perturbation semantics need not transfer to another \cite{baklanov2025fidelity,barkan2026spinrec}, which motivates separating deletion alignment from bounded-intervention alignment rather than treating a single deletion score as explanation validity. ActionShap differs from these protocols in three respects: the perturbation is a declared system-executable weight policy rather than a feature- or event-deletion convention; joint actions are scored against an exact common budget oracle; and inference is performed over distinct users with within-user null calibration.
...
\label{eq:aia}
\end{equation}
Deletion AIA substitutes $\Delta_{u,p}^{z,\mathrm{del}}$; bounded AIA uses $\Delta_{u,p}^{z,\mathrm{bnd}}$, and their difference is the bounded-minus-deletion magnitude-alignment difference (short: the gap). Signed alignment is $\rho_S(-\phi^g_{u,p},\Delta_{u,p}^z)$. For informative non-zero effects $I_u^z=\{p:\Delta_{u,p}^z\ne0\}$, direction accuracy is
\begin{equation}
...
For a method action, define the additive predicted-benefit score (an attribution-ranked action score, not a calibrated estimate of the realized joint effect; Section~\ref{app:replication-sec} quantifies the interaction error of this additivity assumption)
\begin{equation}
...
Before reporting the sensitivities we state their interpretive ceiling: under Amazon full-unseen-catalogue evaluation at the scaled 1,000-user cohort, Monte Carlo Shapley's deletion and bounded correlations are both negative (deletion AIA $-0.211$, bounded AIA $-0.050$; Table~\ref{tab:review5-fullcat}), so its positive bounded-minus-deletion difference ($+0.160$) represents reduced anti-alignment rather than successful bounded-intervention prediction; LIME and leave-one-out instead retain positive bounded alignment ($0.710$ and $0.778$) at catalogue scale. The scaled audit resolves the earlier active-oracle shortage: $883$ of $1{,}000$ users have a positive exact target-margin oracle (NDCG remains sparse: $54$), and decision effects are near-zero for every method ($|\Delta|\le0.002$), so catalogue-scale decision conclusions are reported descriptively. With that caveat, the bounded-minus-deletion difference for Shapley remains positive across the primary ItemKNN candidate, history, and intervention-strength conditions, while absolute values vary. Full-catalogue NDCG alignment is sparse and is treated descriptively. The longer-history conditions also show reduced masking sensitivity, supporting the use of $n_{\max}=20$ as a declared operating boundary rather than an arbitrary computational cap.
...
To test whether the ItemKNN findings transfer to a sequential transformer architecture, the full audit was replicated with a SASRec-style transformer (item and positional embeddings of dimension $64$, one transformer-encoder layer with a single head and dropout $0.1$, history truncated to the most recent $20$ events, trained by leave-one-out BPR with Adam at learning rate $3\times10^{-4}$ for $10$ epochs; the user representation is the history-weighted mean of encoded positions, recomputed at scoring time so that masking/downweighting acts without retraining; Appendix~\ref{app:replication}). Three patterns emerge (Table~\ref{tab:review3}). First, intervention-aware baselines---bounded binary/continuous LIME, finite differences, and integrated gradients, all trained or integrated along the same weight path used at deployment---reach bounded AIA $0.72$--$1.00$ (bounded binary/continuous LIME $0.73$--$0.95$, finite differences and integrated gradients $0.72$--$1.00$), matching or exceeding binary-mask LIME ($0.74$--$0.91$) and Monte Carlo Shapley ($0.14$--$0.70$): part of binary LIME's weakness is therefore a perturbation-distribution mismatch, exactly the confound the review response targets, and Shapley's coalition averaging does not by itself predict bounded interventions better than path-matched baselines. Second, pair interactions $I_{pq}=\Delta(\{p,q\})-\Delta(\{p\})-\Delta(\{q\})$ are small relative to singleton effects for ItemKNN (mean ratio $0.07$; additive-vs-realized pair ranking $\rho=0.84$--$0.90$), supporting the additive decision heuristic of Eq.~(21) empirically under ItemKNN, but are substantial under SASRec (ratios $0.21$--$0.78$, additive-vs-realized $\rho=0.45$--$0.78$), so interaction-aware selection is recommended whenever the scorer is strongly nonlinear. Third, replacing the held-out target by the model's own top-1 recommendation (a prospective, non-target-conditioned audit) leaves Shapley bounded alignment high (run means $0.73$--$0.87$; $93$--$100\%$ of users attain at least $0.5$ alignment among the $126$--$227$ users per run whose generated top-1 lies in the candidate set), showing that the protocol's conclusions do not depend on future-label conditioning. Finally, a component ablation on the same realized-effect currency shows that the protocol components matter: forcing an action (removing the no-action option) changes the selected action in $82$--$89\%$ of users without improving the mean realized effect (mean $\Delta$ in $[-0.023,+0.001]$, with individual deltas reaching $-0.51$ under SASRec), and magnitude-only selection changes the action in $89$--$96\%$ of users and degrades the mean realized effect under the sequential model (mean $\Delta=-0.013$ on Amazon, $-0.041$ on ML-1M), supporting the signed, abstention-aware selection rule of Algorithm~4.

\subsection{Validation extensions (review round 5)}\label{app:review5}
Five validation experiments close remaining review concerns; all release files ship under \texttt{results/review5/}.

\textbf{Full-catalogue scale.} The Amazon full-unseen-catalogue audit was scaled from $250$ to $1{,}000$ users over five seeds (median catalogue $8{,}572$ unseen items; Table~\ref{tab:review5-fullcat}). The active-oracle shortage disappears for the target-margin utility ($883$ positive exact oracles), while NDCG oracles remain sparse ($54$) by discreteness. LIME and leave-one-out retain positive bounded alignment at catalogue scale ($0.710$, $0.778$); Monte Carlo Shapley remains anti-aligned ($-0.050$) with a positive bounded-minus-deletion difference ($+0.160$), confirming that under full catalogues the gap reflects reduced anti-alignment. Realized decision effects are near zero for every method, so catalogue-scale decision quality is reported descriptively.

\textbf{Sequential-model recommendation quality.} The replication SASRec ranks below popularity on both datasets under the minimal cohort recipe (Table~\ref{tab:review5-sasrec-quality}: Amazon NDCG@10 $0.0002$ vs $0.0074$ popularity; ML-1M $0.0019$ vs $0.0191$), and scaling training to the full corpus of eligible users for thirty epochs with a chronological next-item objective does not change the conclusion ($0.0002$ and $0.0015$; the masking gate passes on all ML-1M seeds but only intermittently on Amazon). A competitive sequential recommender therefore requires architectural and hyperparameter tuning beyond this protocol's scope. We report this transparently rather than presenting the sequential model as competitive: the SASRec replication demonstrates attribution transfer under a sequential scorer, and the quality gate's purpose is precisely to prevent such a model from being read as robustness evidence until it passes---consistent with the recommendation that explainer evaluation be gated on recommender quality \cite{mohammadi2024recsys}. Closing the tuning gap (deeper transformer, tuned objectives, or a graph baseline) is declared future work.

\textbf{Exact-Shapley action agreement.} On enumerable users ($n_u\le12$; $300$ Amazon, $73$ ML-1M), the reverse-paired estimator's per-user maximum absolute error has median $1.6\times10^{-4}$ (Amazon) and $1.2\times10^{-3}$ (ML-1M), and the top-2 signed action selected from MC attributions matches the exact-Shapley action with mean Jaccard $0.969$/$0.963$ (median $1.0$ in both) and player-level sign error $0.2\%$/$1.9\%$ (Table~\ref{tab:review5-exact-dist}): MC estimation error does not materially change the selected bounded actions.

\textbf{LIME mask design.} On MovieLens, replacing the seeded Bernoulli-with-replacement masks by distinct masks without replacement, or by full enumeration where $n_u\le9$, changes bounded AIA by $\le0.001$ ($0.906$ vs $0.907$ vs $0.930$; Table~\ref{tab:review5-lime-masks}), so mask duplication is not a practical confound there. On Amazon's shorter histories the spread is larger (Bernoulli $0.669$, unique $0.703$, enumeration $0.776$), but each design is evaluated on its own valid-user subset and constant-vector users are excluded, so the designs are descriptive rather than head-to-head; they are consistent, however, with duplicate masks diluting the design when the distinct coalition space is far smaller than the mask budget, and full enumeration is the recommended design for short profiles in future audits while the frozen Bernoulli design is retained for comparability of the headline results; the ridge penalty is more influential on MovieLens ($\alpha=10$ drops AIA to $0.794$) and mild on Amazon, and $\alpha=1$ is retained with this sensitivity documented.

\textbf{Model vs explainer variance.} For the stochastic profile model, model-initialization variance dominates attribution-seed variance by roughly $7\times$ on Amazon ($0.233$ vs $0.032$ mean per-user variance) and $5\times$ on ML-1M ($0.023$ vs $0.005$; Table~\ref{tab:review5-variance}), confirming that intervals conditional on one frozen model understate total uncertainty and motivating the fixed-model conditioning caveat of Section~\ref{sec:limitations}.

\textbf{User-level convergence quantiles.} A rerun of the convergence audit with per-user recording (100 users per dataset against an independent $M_{\mathrm{pair}}=1000$ reference; Table~\ref{tab:review5-convergence}) shows that the population-mean picture understates the user-level spread. At the final floor $M_{\mathrm{pair}}=500$, the median rank correlation to the reference is $0.991$ on ML-1M and $0.999$ on Amazon, but the 5th percentile is $0.977$ and $0.880$ respectively, and the joint-threshold coverage (rank $\ge0.95$ and action Jaccard $\ge0.80$) is $0.74$ and $0.83$. Even at $M_{\mathrm{pair}}=1000$ coverage reaches only $0.81$ and $0.85$, below a $0.95$ user-universal criterion. This is why the selected budgets are labelled mean-converged: the central tendency is stable well before every individual user clears both thresholds, and the short-history Amazon tail converges more slowly. The coverage-based reading requested by review is therefore reported alongside, not in place of, the mean diagnostic.
...
Two datasets and two primary-model families do not cover graph recommenders, multimodal inputs, online feedback, provider objectives, or heterogeneous intervention costs; the SASRec replication (Appendix~\ref{app:replication}) covers one sequential transformer but a broader architecture sweep remains future work. Full-catalogue evaluation is now scaled to $1{,}000$ users on Amazon ($883$ active target-margin oracles; Appendix~\ref{app:review5}), while the MovieLens full-catalogue cell remains smaller; the seeded Bernoulli LIME mask design is validated against without-replacement and enumeration designs with negligible difference (Appendix~\ref{app:review5}). The replication SASRec ranks below popularity under both the cohort recipe and full-corpus thirty-epoch training, so sequential-architecture evidence is limited to attribution transfer until the tuning gap is closed; this boundary and the profile model's quality shortfall (one failed gate) are retained as explicit boundaries rather than generalized.
