...
Two directional hypotheses are predeclared. \textbf{H1}: deletion alignment and bounded-intervention alignment are different estimands; methods whose construction is local to the deletion neighbourhood (leave-one-out, locally weighted LIME) should score high on deletion alignment but lose alignment under bounded downweighting, whereas coalition-averaged Shapley values should be less specialised to either perturbation semantics. \textbf{H2}: pointwise alignment does not determine joint decision quality, so method rankings under AIA and under NDCG regret may disagree. The primary bounded intervention uses $\rho=0.5$ as the declared operating point: it lies strictly between deletion ($\rho=0$) and the identity ($\rho=1$), is large enough to move ranked lists above the masking-gate threshold, and represents a bounded discount of historical evidence rather than its removal; $\rho=0.25$ is a predeclared sensitivity that probes the ordering of the bound.
...
\end{tikzpicture}
\caption{ActionShap evaluation architecture. The fixed vertical path separates player construction, model scoring, attribution, intervention execution, and outcome measurement. Side panels show controls held constant across methods; the stability diagnostics referenced in the final stage are reported in Table~D3.}
...
\end{equation}
$\mathrm{Prec}@k$ is a compound metric: a predicted top-$k$ player is credited only when the realized bounded effect is positive \emph{and} directionally consistent; it is therefore stricter than rank agreement. If $n_u=0$ the quantity is missing. For a selected joint action $\widehat A_{u,g}$, success is $\mathbf{1}[\Delta_u^z(\widehat A_{u,g})>0]$; a negative realized effect identifies a harmful action, and abstention is $\mathbf{1}[\widehat A_{u,g}=\varnothing]$. Rank stability over $R_{\mathrm{seed}}$ seed runs is
\begin{equation}
\mathrm{Stab}_u(g)=\frac{2}{R_{\mathrm{seed}}(R_{\mathrm{seed}}-1)}\sum_{r<s}\rho_S(\boldsymbol\phi_{u}^{g,r},\boldsymbol\phi_{u}^{g,s}),
\end{equation}
...
\Statex\hspace{1em}\textit{(sorting strictly positive individual benefits and taking at most $B$ equals enumerating $\mathcal A_{u,B}$, since the predicted benefit is additive with unit action costs)}
...
\caption{Independent Monte Carlo convergence against an $M_{\mathrm{pair}}=1000$ ($T=2000$ orders) reference. Left: rank agreement with the reference; right: signed $B=2$ action-set Jaccard overlap, the second selection criterion; dashed lines mark the $0.95$ and $0.80$ thresholds. The legend maps every curve to its dataset--model--utility condition. Efficiency is not used as a convergence criterion because the prefix walk telescopes.}
...
No single closest work combines these components. Counterfactual recommendation methods generate per-method profile edits and evaluate them under method-specific objectives, without a common declared feasible policy, a utility-specific joint-action oracle with a no-action option, or abstention; counterfactual explanation-evaluation benchmarks audit consistency, sparsity, or effectiveness of counterfactual sets, but do not separate deletion from bounded perturbations, do not calibrate alignment against a within-user chance null, and do not perform seed-averaged distinct-user paired inference. ActionShap is therefore neither a new counterfactual generator nor a new explainer: its non-overlap with the works in Table~\ref{tab:related-work-comparison} lies in auditing any attribution ranking against a declared bounded intervention policy on a common exact action space, with signed benefit selection, abstention, null calibration, and distinct-user inference held fixed across methods.
...
\begin{tabular}{@{}*{6}{>{\raggedright\arraybackslash}p{.155\textwidth}}@{}}
\toprule
Work & Players & Policy & Outcome & Oracle & Inference \\
\midrule
PRINCE \cite{ghazimatin2020} & Provider, profile & Counter-factual gen. & Rec.\ change & Method-spec. & Method-spec. \\
ACCENT \cite{tran2021} & User factors & Counter-factual masking & Top-$K$ expl. & None & Method-spec. \\
Tan et al. \cite{tan2021} & Rec.\ factors & Counter-factual expl. & Rank change & Method-spec. & User/item \\
CEERS \cite{baklanov2024} & Expl.\ factors & Counter-factual eval. & Expl.\ quality & Benchmark-spec. & Repro.\ protocol \\
Barkan et al. \cite{barkan2024} & Learned masks & Model-spec.\ counter-factual & Expl.\ quality & Learned obj. & Method-spec. \\
LXR \cite{gurevitch2025} & Learned expl.\ masks & Model-spec.\ counter-factual & Expl.\ quality & Learned obj. & Method-spec. \\
Mohammadi et al. \cite{mohammadi2025} & Expl.\ factors & Top-$K$ counter-factual eval. & Consistency & None & Benchmark \\
Nguyen et al. \cite{nguyen2026} & User/profile & Top-$K$ counter-factual eval. & Effect., sparsity & Benchmark-spec. & Cross-paper \\
ActionShap & History interactions & Bounded downweight & AIA, NDCG, regret & Exact $B=2$; abstain & Distinct-user \\
\bottomrule
\end{tabular}
\end{table}
