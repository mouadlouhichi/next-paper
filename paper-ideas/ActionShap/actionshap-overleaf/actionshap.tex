...
\end{equation}
For a selected joint action $\widehat A_{u,g}$, success is $\mathbf{1}[\Delta_u^z(\widehat A_{u,g})>0]$; a negative realized effect identifies a harmful action, and abstention is $\mathbf{1}[\widehat A_{u,g}=\varnothing]$. Rank stability over $R_{\mathrm{seed}}$ seed runs is
\begin{equation}
...
For each seed we compute user-level quantities first; repeated seed values are then averaged within distinct user before bootstrap resampling. User bootstrap intervals are two-sided percentile intervals from 10,000 user resamples. For a paired metric, $d_u=x_u-y_u$ and $T_{\mathrm{obs}}=n^{-1}\sum_{u=1}^{n}d_u$. Paired plus-one sign permutations use 10,000 two-sided sign flips $T_r^*=n^{-1}\sum_u\epsilon_{u,r}d_u$, with $\epsilon_{u,r}\in\{-1,+1\}$ and $p=(1+\#\{|T_r^*|\ge|T_{\mathrm{obs}}|\})/(R+1)$. Holm correction is applied separately within each dataset--model--condition--metric family across the declared method-pair comparisons. Paired Cohen's $d_z$ is $\bar d/s_d$, where $d_u$ is the user-level paired difference and $s_d$ its sample standard deviation. The paired comparison uses the intersection of users with finite values for both methods; missing correlations are retained as missing. Active-oracle counts define the denominator for normalized regret. No coalition evaluation is an independent statistical observation.
...
All paths in generated manifests are repository-relative and all release archives are verified by SHA-256. Code, configuration files, validation reports, preprocessing/run scripts, and machine-readable result matrices are included in the package's \texttt{release/} directory and the repository release. This manuscript and supplement are generated from source commit \texttt{5ca120b}; the schema-v2 result archive has SHA-256 \texttt{ac4c7fb1993458b6b41054974ebff215710e7a8b5894c7aa6af828e94b2a5b0f}. The source repository URL is https://github.com/mouadlouhichi/next-paper; its public visibility and any DOI-backed deposit remain external release actions.
...
Table~\ref{tab:related-work-comparison} compares the protocol dimensions most relevant to the closest counterfactual and explanation-evaluation studies. ActionShap differs not by claiming a new causal estimator, but by combining interaction-player attribution, a declared bounded profile policy, a utility-specific joint oracle, abstention, and distinct-user inference.
...
\begin{tabular}{@{}p{.16\textwidth}p{.18\textwidth}p{.18\textwidth}p{.16\textwidth}p{.14\textwidth}p{.10\textwidth}@{}}
\toprule
Work & Players & Intervention/policy & Outcome & Joint oracle & Inference \\
\midrule
PRINCE \cite{ghazimatin2020} & Provider/profile & Counterfactual generation & Recommendation change & Method-specific & Method-specific \\
ACCENT \cite{tran2021} & User factors & Counterfactual masking & Top-$K$ explanation & No common budget oracle & Method-specific \\
Tan et al. \cite{tan2021} & Recommendation factors & Counterfactual explanation & Ranking change & Method-specific & User/item evaluation \\
CEERS \cite{baklanov2024} & Explanation factors & Counterfactual evaluation & Explanation quality & Benchmark-specific & Reproducibility protocol \\
Barkan et al. \cite{barkan2024} & Learned masks & Model-specific counterfactual & Explanation quality & Learned objective & Method-specific \\
LXR \cite{gurevitch2025} & Learned explanation masks & Model-specific counterfactual & Explanation quality & Learned objective & Method-specific \\
Mohammadi et al. \cite{mohammadi2025} & Explanation factors & Top-$K$ counterfactual evaluation & Consistency & No common budget oracle & Benchmark study \\
Nguyen et al. \cite{nguyen2026} & User/profile factors & Top-$K$ counterfactual evaluation & Effectiveness, sparsity & Benchmark-specific & Cross-paper benchmark \\
ActionShap & History interactions & $w_p\leftarrow\rho w_p$ & AIA, NDCG, regret & Exact $B=2$; abstention & Distinct-user paired inference \\
\bottomrule
\end{tabular}
\end{table}
