...
\textbf{LIME mask design.} Replacing the seeded Bernoulli-with-replacement masks by distinct masks without replacement, or by full enumeration where $n_u\le9$, changes MovieLens bounded AIA by $\le0.001$ ($0.906$ vs $0.907$ vs $0.930$; Table~\ref{tab:review5-lime-masks}), so mask duplication is not a practical confound; the ridge penalty matters more ($\alpha=10$ drops AIA to $0.794$), and $\alpha=1$ is retained with this sensitivity documented.
