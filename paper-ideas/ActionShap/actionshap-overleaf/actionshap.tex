...
\end{equation}
where $y$ is the temporal target, $L=10$, and $\sigma(t)=(1+e^{-t})^{-1}$ has unit temperature. The operational ranking outcome is
\begin{equation}
...
The real result archive contains 80 recommendation runs plus five convergence studies. All assets in this manuscript are generated from schema-v2 raw results with repository-relative paths and content hashes. The primary ItemKNN target-margin analysis uses $M_{\mathrm{pair}}=500$ base permutations and $T=1000$ evaluated orders; the independent convergence study selects lower diagnostic values for some conditions, while the conservative final floor is retained.
...
\safeinput{tables/protocol_audit.tex}
...
Target-margin convergence selects $M_{\mathrm{pair}}=50$ for Amazon ItemKNN, $M_{\mathrm{pair}}=100$ for Amazon profile, $M_{\mathrm{pair}}=250$ for MovieLens ItemKNN, and $M_{\mathrm{pair}}=50$ for MovieLens profile; the final suite retains the conservative floor $M_{\mathrm{pair}}=500$ ($T=1000$ evaluated orders). The NDCG attribution sensitivity remains unconverged at $M_{\mathrm{pair}}=1000$ and is reported only as a stress test.
...
\section{Expanded robustness results}\label{app:robustness}

The primary ItemKNN result is intentionally complemented by a matrix of candidate-size, history-length, intervention-strength, and budget sensitivities. Candidate-size changes test whether the pointwise alignment is an artefact of a 200-item sampled pool. History-length changes test whether the declared $n_{\max}=20$ window is an operational boundary. The $\rho=0.25$ condition tests whether the bounded action remains separable from deletion. Budgets one and three are reported only for joint decision outcomes because they do not define new singleton gap estimands.

\safeinput{tables/sensitivity_results.tex}
\safeinput{tables/full_catalogue_results.tex}
\safeinput{tables/actionability_gap_robustness.tex}
The complete paired gap-advantage comparisons, including effect sizes and Holm-adjusted $p$-values, are supplied in
\texttt{tables/actionability_gap_advantage.csv}; they are not duplicated as a dense PDF export.

\section{Statistical reporting and null calibration}\label{app:statistics}

The statistical unit is a distinct user. For a method metric $m$, seed-level values are averaged within user only after the matched seed computations are complete. Missing AIA values are retained as missing when a vector is constant or non-finite. Normalized regret is conditional on a positive oracle effect and is reported with the active-oracle count; the exact action-space invariant requires it to be non-negative up to numerical tolerance. The random control is a negative control for absolute alignment and action selection; its gap is not interpreted as a validity measure. Full user--seed metrics and paired comparisons are supplied as CSV/JSON supplementary assets rather than repeated as dense PDF tables.


\section{Reproducibility and release manifest}\label{app:release}

The result release contains 85 JSON files: 80 recommendation runs and five convergence studies. It records source hashes, configuration values, seed assignments, gate diagnostics, selected users, per-user effects, attributions, actions, and oracle outcomes. The raw archive is distributed separately because the datasets and raw user-level records are not ordinary manuscript assets. The final validation manifest is the gate for numerical claims; its notes, including one nonblocking MovieLens profile gate failure and NDCG stress-test non-convergence, are part of the evidence rather than suppressed warnings.
...
\end{verbatim}
All paths in generated manifests are repository-relative and all release archives are verified by SHA-256. The source repository URL is https://github.com/mouadlouhichi/next-paper; it must be made public or mirrored before submission. The exact source commit, dependency versions, input hashes, and generated-file hashes are recorded by the release manifest and the companion reproducibility note. No DOI is claimed for the local checkout; the immutable artifact identifier is the SHA-256 release archive until a DOI-backed deposit is made.

\section{Additional limitations}\label{app:limitations}

ActionShap evaluates a retrospective target-conditioned audit. The held-out target is used to define the labelled instance and ranking utility, so the protocol is not a prospective action policy. The bounded intervention changes inference-time profile influence without retraining; it is system-executable but not necessarily user-executable. Candidate target inclusion gives coverage one by construction, while full-catalogue results use a smaller subset. The profile model is deliberately retained as a quality and masking-sensitivity boundary. These restrictions define the scope of the conclusions and should accompany any reuse of the artifact.

\clearpage
\section{Expanded Asset Tables and Statistical Audit}\label{app:expanded-audit}

This appendix exposes the complete numerical views behind the compact tables in the main text. It is intentionally long: the purpose is auditability, not a second narrative results section. Rows are generated from the schema-v2 release and are not manually transcribed. All means use the distinct-user aggregation specified in the main text; confidence intervals and missing-user counts remain in the CSV release.

\subsection{Complete alignment matrix}
\input{tables/appendix_aia_full.tex}

\subsection{Complete joint-intervention outcomes}
\input{tables/appendix_intervention_full.tex}

\subsection{Selected paired comparisons}
\input{tables/appendix_paired_tests.tex}

\subsection{Convergence matrix}
\input{tables/appendix_convergence_full.tex}

\subsection{Executable contract}
\input{tables/appendix_contract.tex}

\section{Reproducibility and release record}\label{app:release-record}

The Overleaf folder is a presentation package, not the raw experiment directory. The source archive contains the frozen runner, requirements lockfile, schema-v2 result bundle, source hashes, and validation report. The archive SHA-256 is recorded in the repository release metadata. The manuscript build uses the exact Springer Nature class and numeric Math--Physical Sciences bibliography style supplied by the working SignalShap Overleaf package. The main PDF intentionally includes only compact tables; the full user--seed vectors and paired-test matrix are retained outside the manuscript source so that readers can audit the calculation without forcing the publication layout to contain a raw data frame.

\subsection{Submission checklist}
\begin{enumerate}
\item Compile with pdfLaTeX and BibTeX using the included \texttt{latexmkrc};
\item confirm that all 47 ActionShap citation keys resolve to numbered bibliography entries;
\item confirm that the first page has one title, separated authors, numbered affiliation, corresponding-author email, and a labelled Keywords line;
\item confirm that Tables~\ref{tab:protocol-audit}, \ref{tab:aia-components}, and \ref{tab:intervention-outcomes} appear once in the main text and that the appendix tables are not duplicated;
\item report the exact release commit and artifact hash with any public code deposit.
\end{enumerate}
