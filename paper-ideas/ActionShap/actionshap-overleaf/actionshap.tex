...
\end{equation}
Let $T_{u,k}^g$ be the $k$ players with largest predicted benefit $-\phi^g$; top-$k$ intervention precision is
\begin{equation}
\mathrm{Prec}@k_u^z(g)=k^{-1}\sum_{p\in T_{u,k}^g}\mathbf{1}\left[\Delta_{u,p}^z>0\ \wedge\ \operatorname{sign}(-\phi^g_{u,p})=\operatorname{sign}(\Delta_{u,p}^z)\right].
\end{equation}
For a selected joint action $\widehat A_{u,g}$, success and harm are $\mathbf{1}[\Delta_u^z(\widehat A_{u,g})>0]$ and $\mathbf{1}[\Delta_u^z(\widehat A_{u,g})<0]$, while abstention is $\mathbf{1}[\widehat A_{u,g}=\varnothing]$. Rank stability over $R$ seed runs is
\begin{equation}
\mathrm{Stab}_u(g)=\frac{2}{R(R-1)}\sum_{r<s}\rho_S(\boldsymbol\phi_{u}^{g,r},\boldsymbol\phi_{u}^{g,s}),
\end{equation}
...
LXR \cite{barkan2024,gurevitch2025} & Learned masks & Model-specific counterfactual & Explanation quality & Learned objective & Method-specific \\
