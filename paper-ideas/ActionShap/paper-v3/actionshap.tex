...
%%%% Springer Nature journal packages, following the working SignalShap source
\usepackage{graphicx}
\usepackage{multirow}
...
%%%% Theorem-like environments used by the formal appendix
...
\end{equation}
The reverse orders are antithetic variance-reduction partners and coalition states are cached. In the executable configuration, `permutations' denotes $M_{\mathrm{pair}}=500$, hence $T=1000$ evaluated orders. The comparison includes locally weighted binary-mask LIME, LOO, greedy sequential deletion, and random attribution. No method observes measured intervention effects, oracle actions, or aggregate test outcomes.
...
\label{tab:contract}
\begin{tabular}{@{}p{.18\textwidth}p{.36\textwidth}p{.38\textwidth}@{}}
\toprule
...
\texttt{final/tables/actionability_gap_advantage.csv}; they are not duplicated as a dense PDF export.
