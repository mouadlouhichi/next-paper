...
\caption{Primary signed diagnostic metrics for the ItemKNN target-margin analysis. This is a descriptive diagnostic summary; the confirmatory inferential targets are the paired comparisons in Table~\ref{tab:paired-full}. Direction accuracy (main-manuscript Eq.~17) and top-$3$ precision use signed bounded effects, Precision@3 is the compound positive-and-correct-direction metric defined in main-manuscript Section~4.3, signed alignment is the Spearman correlation $\rho_S(-\phi^g_{u,p},\Delta^{z,\mathrm{bnd}}_{u,p})$ between predicted benefits and bounded effects (seed-averaged per user, 10,000-draw bootstrap CIs), and stability is rank stability across the five seed runs. Direction accuracy and signed alignment are different quantities: the former is the fraction of non-zero effects whose sign is predicted correctly; the latter is a rank correlation over all players.}
