> **STATUS: HISTORICAL DESIGN NOTE.** The canonical manuscript is `paper-v3/actionshap.tex`. The older 11-condition/22-comparison narrative below is retired; current assets use nine singleton ItemKNN conditions, report all five methods, and treat the Actionability Gap as descriptive.
...
Recommendation explanations are usually evaluated by deletion-based faithfulness, although deployed systems act through feasible modifications. ActionShap separates absolute alignment from intervention robustness. Empirically, Shapley is the only evaluated method whose alignment improves consistently when deletion is replaced by bounded intervention, even though local baselines retain higher absolute alignment.
...
5. **Intervention-robustness analysis.** The current release reports nine singleton ItemKNN conditions with deletion AIA, bounded AIA, gap, null calibration, and decision outcomes; no unique-superiority claim is made.
6. **Reproducible artifact.** One notebook wrapper over tracked scripts, deterministic temporal splits, fixed target-plus-unseen-negative evaluation sets, frozen intervention policy, independent convergence analysis, hierarchical statistics, and content-addressed generated assets.
