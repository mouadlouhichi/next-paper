...
def main() -> None:
    ...
    parser.add_argument("--paper", default="actionshap.tex")
    parser.add_argument("--bib", default="actionshap-bibliography.bib")
    parser.add_argument("--require-final", action="store_true")
    args = parser.parse_args()
    package_root = Path(__file__).resolve().parents[2]
    paper_path = (package_root / args.paper).resolve()
    bib_path = (package_root / args.bib).resolve()
    ...
    if "\\bibliography{actionshap-bibliography}" not in tex:
        errors.append("manuscript does not point to actionshap-bibliography.bib")
    ...
    if "ActionShap: Evaluating Recommendation Attributions under Bounded Profile Interventions" not in tex:
    ...
    validation_path = paper_root / "release" / "validation_report.json"
    validation = (
