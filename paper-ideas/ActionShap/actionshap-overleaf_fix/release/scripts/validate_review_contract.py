#!/usr/bin/env python3
"""Reviewer-facing semantic and production checks for the ActionShap package."""
...
def main() -> None:
    ...
    paper_root = resolve_paper_root(package_root, args.paper_root)
    tables = paper_root / "release" / "matrices"
    tex_tables = paper_root / "tables"
    ...
    if "depends only on $S$" not in tex:
        errors.append("player-window scoring semantics are not explicit")
    if "history-conditioned recommender interface" not in tex.lower():
