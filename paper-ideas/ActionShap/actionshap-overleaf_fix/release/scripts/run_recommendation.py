...
# The manuscript bundle contains reviewer-facing entry points and matrices; the
# executable ``actionshap`` package lives in the full public repository checkout.
code_root = Path(__file__).resolve().parents[1]
if not (code_root / "actionshap").is_dir():
    raise SystemExit(
        "The compact manuscript bundle does not duplicate the actionshap package. "
        "Run this entry point from paper-ideas/ActionShap/code in the public "
        "repository checkout."
    )
