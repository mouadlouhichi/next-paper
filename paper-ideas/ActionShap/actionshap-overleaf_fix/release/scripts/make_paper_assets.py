...
PRIMARY_METRICS = [
    ...
    "intervention_success_ndcg_active",
    "harm_ndcg",
]
            ...
            for method, raw_values in user["methods"].items():
                ...
                row = {
                    **base,
                    ...
                    "intervention_success_ndcg_active": (
                        float(bool(values.get("success_ndcg")))
                        if values.get("normalized_regret_ndcg") is not None
                        else None
                    ),
                    "harm_ndcg": (
                        float(values.get("effect_ndcg") < 0)
                        if values.get("effect_ndcg") is not None
                        else None
                    ),
                    "abstention": float(
...
def write_compact_intervention_table(summary: pd.DataFrame, path: Path) -> None:
    ...
    lines = [
        ...
        r"\caption{Primary budget-two NDCG decisions. Success is shown over all users and conditional on a positive exact oracle; Harm is the fraction of repetition--user actions with negative realized effect. NRegret is conditional on active users.}",
        r"\label{tab:intervention-outcomes}",
        r"\begin{tabular}{llrrrrrr}",
        r"\toprule",
        r"Dataset & Method & $\Delta$NDCG & Succ. & Succ.$\mid O>0$ & Harm & Abst. & NRegret ($n$) \\",
        r"\midrule",
    ]
    for dataset, label in (("MovieLens-1M", "MovieLens"), ("Amazon-Digital-Music", "Amazon")):
        for method in methods:
            ...
            active_success = _summary_value(summary, dataset=dataset, model="itemknn", role="primary", condition="primary", method=method, metric="intervention_success_ndcg_active")
            harm = _summary_value(summary, dataset=dataset, model="itemknn", role="primary", condition="primary", method=method, metric="harm_ndcg")
            ...
            if any(value is None for value in (effect, success, active_success, harm, abstention, regret)):
                continue
            success_text = _tex_number(100 * success["mean"], 1) + r"\%"
            active_success_text = _tex_number(100 * active_success["mean"], 1) + r"\%"
            harm_text = _tex_number(100 * harm["mean"], 1) + r"\%"
            abstention_text = _tex_number(100 * abstention["mean"], 1) + r"\%"
            lines.append(
                f"{label} & {labels[method]} & {_tex_number(effect['mean'])} & "
                f"{success_text} & {active_success_text} & {harm_text} & {abstention_text} & "
                f"{_tex_number(regret['mean'])} ({int(regret['n_users'])}) \\\\"
            )
        if label == "MovieLens":
            lines.append(r"\midrule")
    lines += [r"\bottomrule", r"\end{tabular}", r"\end{table}", ""]
...
def write_compact_gap_table(summary: pd.DataFrame, path: Path) -> None:
    ...
    lines = [
        ...
        r"\begin{table}[t]\centering\scriptsize",
        r"\caption{Bounded-minus-deletion magnitude-alignment difference across predeclared ItemKNN conditions.}",
...
def make_actionability_gap_assets(
    ...
    for idx in gap.index:
        ...
        try:
            ...
            pass
    gap.to_csv(table_root / "actionability_gap_robustness.csv", index=False)
    write_tex(gap[["condition_label", "method_label", "mean", "ci95_low", "ci95_high", "n_users", "missing_users"]], table_root / "actionability_gap_robustness.tex", "Magnitude-alignment Gap defined as bounded AIA minus deletion AIA; budgets excluded.", "tab:gap-robustness")
...
in some. The magnitude-alignment Gap is therefore a descriptive perturbation-
...
"""
    ...
    if not gap.empty:
        ...
        fig.suptitle("AIA components and bounded-minus-deletion difference (singleton estimand)", y=1.01)
        fig.tight_layout()
...
def main() -> None:
    ...
    write_tex(
        ...
        "Within-user, within-repetition permutation calibration of target-margin AIA.",
        "tab:aia-null",
    )
    write_tex(
        primary_summary,
    ...
    write_tex(
        ...
        "Attribution rank stability across repeated runs.",
        "tab:stability",
    )
    ...
    manifest = {
        ...
        "repository": {
            ...
            "visibility_required_before_submission": "repository_public",
