...
\graphicspath{{paper-v2/final/figures/}}
...
\safeinput{paper-v2/final/tables/protocol_audit.tex}
...
\safeinput{paper-v2/final/tables/aia_components.tex}
\safeinput{paper-v2/final/tables/aia_permutation_null.tex}
...
\safeinput{paper-v2/final/tables/intervention_outcomes.tex}
\safeinput{paper-v2/final/tables/paired_tests.tex}
...
\safeinput{paper-v2/final/tables/convergence.tex}
\safeinput{paper-v2/final/tables/attribution_stability.tex}
...
\safeinput{paper-v2/final/tables/sensitivity_results.tex}
\safeinput{paper-v2/final/tables/full_catalogue_results.tex}
...
\begin{table}[t]
\centering\small
\caption{Frozen experimental contract used for every paper-eligible run.}
\label{tab:contract}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\midrule
Datasets & MovieLens-1M; Amazon Digital Music & same\\
Models & ItemKNN; profile aggregator & full catalogue and profile boundary\\
Temporal split & last test, penultimate validation & deterministic record-index ties\\
Primary users & 1,000 distinct users & 250 matched users\\
Players & most recent 20 training interactions & 50 and 100 history caps\\
Candidates & target + 199 unseen negatives & 100, 500, full unseen catalogue\\
Attribution utility & target margin & NDCG stress test\\
Intervention & downweight $\rho=0.5$ & deletion and $\rho=0.25$\\
Budget & exact $B=2$ & $B=1$ diagnostic, $B=3$ sensitivity\\
Seeds & 42,43,44,45,46 & candidate/user/tie seeds independent\\
Inference & seed-average within user & user bootstrap and paired tests\\
\bottomrule
\end{tabular}}
\end{table}
...
\safeinput{paper-v2/final/tables/sensitivity_results.tex}
\safeinput{paper-v2/final/tables/full_catalogue_results.tex}
\safeinput{paper-v2/final/tables/actionability_gap_robustness.tex}
\safeinput{paper-v2/final/tables/actionability_gap_advantage.tex}

\section{Statistical reporting and null calibration}\label{app:statistics}

The statistical unit is a distinct user. For a method metric $m$, repeated seed values are averaged within user before bootstrap resampling. Missing AIA values are retained as missing when a vector is constant or non-finite. Normalized regret is conditional on a positive oracle effect and is reported with the active-oracle count. The random control is a negative control for absolute alignment and action selection; its gap is not interpreted as a validity measure.
...
The canonical artifact directory is \texttt{paper-ideas/ActionShap/paper-v2/}. The reproducibility command is:
\begin{verbatim}
python scripts/make_paper_assets.py --raw results/raw --out ../paper-v2
\end{verbatim}
All paths in generated manifests are repository-relative and all release archives are verified by SHA-256.
