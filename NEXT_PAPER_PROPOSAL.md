...
> **Observation-propensity vs. action-propensity — a substitution to justify explicitly in the paper (flagged by the reviewer, not a blocker).** Strictly, IPS/DR need an *action/intervention* propensity: the probability that the logging policy would have produced `do(S)`. A Schnabel-style `π̂_0` is an *observation* propensity: the probability that the `(u,i)` rating was logged at all. These are **not the same object**, and the paper must say why the former is a defensible proxy for the latter rather than presenting them as identical. The justification to write down: in these datasets an intervention is *defined by* a recorded interaction at the profile level (mask/downweight a logged history item — see §3.6 "intervention mechanism"), so "this interaction was observed/logged" is the only signal available for whether the corresponding lever state ever occurred; where a full action-propensity is unavailable (no randomisation), the observation propensity is the standard, explicitly-acknowledged stand-in used throughout unbiased-LTR (Schnabel et al., 2016; Saito et al.). This is a *modelling assumption*, stated as such, and it is exactly the kind of limitation the synthetic known-propensity arm and the DR-robustness analysis are designed to bound: on synthetic data the two propensities coincide and are known, giving a positive control for the substitution, and on real data we report how conclusions shift under propensity misspecification. The paper should state this substitution in one or two sentences in the OPE section and cite it in Limitations.
